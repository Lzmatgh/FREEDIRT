    ...
    public class TVEPrefabConverter : EditorWindow
    {
        ...
        void CreateMeshInstances()
        {
            ...
            for (int i = 0; i < meshFiltersInPrefab.Count; i++)
            {
                ...
                if (mesh != null)
                {
                    var isReadable = TVEUtils.MarkMeshReadable(mesh);

                    var meshInstance = Instantiate(mesh);
                    meshInstance.name = mesh.name + " " + i.ToString("X2") + " (" + outputSuffix + " Model)";
                    meshInstances.Add(meshInstance);

                    TVEUtils.SetMeshReadable(mesh, isReadable);
                }
                else
                {
        ...
        void CreateColliderInstances()
        {
            ...
            for (int i = 0; i < meshCollidersInPrefab.Count; i++)
            {
                ...
                if (mesh != null)
                {
                    ...
                    colliderInstance.name = mesh.name + " " + i.ToString("X2") + " (" + outputSuffix + " Model)";
