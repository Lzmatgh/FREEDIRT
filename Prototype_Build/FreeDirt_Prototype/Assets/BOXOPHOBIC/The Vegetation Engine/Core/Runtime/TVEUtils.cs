...
namespace TheVegetationEngine
{
    public class TVEUtils
    {
...
#if UNITY_EDITOR
        public static bool MarkMeshReadable(Mesh mesh)
        {
            var isReadable = true;
        ...
        public static void SetMeshReadable(Mesh mesh, bool isReadable)
        {
            var meshPath = AssetDatabase.GetAssetPath(mesh);
            var modelImporter = AssetImporter.GetAtPath(meshPath) as ModelImporter;
        ...
        {
            var meshPath = AssetDatabase.GetAssetPath(mesh);
            var isReadable = MarkMeshReadable(mesh);
            ...
            SetMeshReadable(mesh, isReadable);

            var basePath = meshPath.Replace("TVE Model", "Export Base");
            var motionPath = meshPath.Replace("TVE Model", "Export Motion");
